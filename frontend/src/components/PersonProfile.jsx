...
export default function PersonProfile({
  ...
  const canAskRelated = Boolean(selfPersonId && onHowRelated && person.id !== selfPersonId)
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center space-x-4 min-w-0">
              ...
              <div className="min-w-0">
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 break-words">{person.name}</h2>
                {person.nickName && <p className="text-lg text-slate-600 italic">&quot;{person.nickName}&quot;</p>}
              </div>
            </div>
            <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 text-2xl flex-shrink-0 ml-2" aria-label="Close">
