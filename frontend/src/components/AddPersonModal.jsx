    ...
    <div className="modal-overlay fixed inset-0 flex items-center justify-center z-50 p-4">
      <div className="modal-content bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-thin">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-slate-800">Add member</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input name="name" value={formData.name} onChange={handleChange} required className="w-full p-3 border border-slate-300 rounded-lg" placeholder="John Doe" />
          </div>
          <div>
          ...
          <div className="border-t pt-4">
            ...
            <div className="flex space-x-2 mb-4">
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg font-semibold ${uploadMode === 'upload' ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}`}
              >
                Upload File
              </button>
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg font-semibold ${uploadMode === 'url' ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}`}
              >
                Photo URL
              </button>
            </div>
            ...
            {uploadMode === 'upload' ? (
              ...
              >
                ...
                <p className="text-slate-600 font-semibold mb-1">Click to upload or drag and drop</p>
                <p className="text-sm text-slate-500">PNG, JPG, GIF or WebP (max 5MB)</p>
                <input ref={fileInputRef} type="file" accept="image/*" onChange={(e) => handleFile(e.target.files[0])} className="hidden" />
              </div>
            ) : (
              <input type="url" name="photoUrl" value={formData.photoUrl} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg" placeholder="https://example.com/photo.jpg" />
            )}

            {previewUrl && <img src={previewUrl} alt="Preview" className="mt-4 w-32 h-32 rounded-lg object-cover border-2 border-slate-200" />}
            {uploadMode === 'url' && formData.photoUrl && (
              <img src={formData.photoUrl} alt="Preview" className="mt-4 w-32 h-32 rounded-lg object-cover border-2 border-slate-200" onError={(e) => { e.target.style.display = 'none' }} />
            )}
          </div>
